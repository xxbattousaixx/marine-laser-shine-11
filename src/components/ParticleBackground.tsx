...
function Particles({ count = 2000, color = '#00d4ff' }: ParticlesProps) {
  ...
  return (
    <div className={`absolute inset-0 -z-10 ${className}`}>
      ...
      >
        ...
        <Particles count={1500} color={config.particles} />
        <FloatingShape position={[-3, 2, -5]} color={config.shapes[0]} speed={0.8} />
        <FloatingShape position={[3, -1, -4]} color={config.shapes[1]} speed={1.2} />
        <FloatingShape position={[0, 3, -6]} color={config.shapes[0]} speed={0.6} />
        <FloatingShape position={[-2, -2, -3]} color={config.shapes[1]} speed={1} />
      </Canvas>
    </div>
  );
};
